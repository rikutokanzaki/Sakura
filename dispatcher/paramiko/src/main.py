...
class SSHProxyServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username: str, password: str) -> int:
        ...
        if self.mode == "standalone":
            try:
                ...
                logger.exception(
                    "Failed to record login via cowrie_connector (standalone mode)"
                )
        else:
            try:
                self.heralding_connector.record_login(
        ...
        log_event.log_auth_event(
            self.client_addr, HOST, PORT, username, password, auth_success, self.mode
        )
    ...
    def check_channel_exec_request(self, channel, command) -> bool:
        ...
        self.event.set()
        threading.Thread(
            target=self._handle_exec_request, args=(channel, command), daemon=True
        ).start()
        return True
    ...
    def _handle_exec_request(self, channel, command):
        try:
            ...
            log_event.log_command_event(
                src_ip, src_port, self.username, command_str, "~", self.mode
            )
            ...
            if self.mode in ("static", "standalone"):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        logger.error(
                            "Failed to start cowrie at exec request (HTTP %s)",
                            res.status_code,
                        )
            ...
            try:
                output = self.cowrie_connector.execute_command_via_shell(
                    command_str, self.username, self.password
                )
    ...
    def _trigger_cowrie(self):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    "Failed to start cowrie at auth (HTTP %s)", res.status_code
                )
        except Exception:
    ...
    try:
        ...
        threading.Thread(
            ...
            args=(
                ...
                addr,
                start_time,
                server.cowrie_launched,
                server.cowrie_connector,
                server.mode,
            ),
