...
class PromptManager:
    def __init__(
        self, launcher_host="launcher", cowrie_host="cowrie", cowrie_port=2222
    ):
    ...
    def get_cowrie_prompt(self, username, password) -> str:
        ...
        try:
            res = requests.post(
                f"http://{self.launcher_host}:5000/trigger/cowrie", timeout=5
            )
            ...
            resource_manager.close_ssh_connection(
                client=client, shell=shell, transport=transport
            )
