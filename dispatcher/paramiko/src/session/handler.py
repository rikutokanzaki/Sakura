...
from utils import ansi_sequences, log_event, resource_manager, set_motd
...
def handle_session(
    ...
    addr: tuple,
    start_time: float,
    cowrie_launched: bool,
    cowrie_connector: connect_server.SSHConnector,
    mode: str,
    transport=None,
    client_socket=None,
) -> None:
    ...
    reader = line_reader.LineReader(
        chan, username, password, prompt, history, cowrie_connector
    )
        ...
        while True:
            ...
            if not cowrie_launched:
                if mode == "dynamic" or mode == "rotate":
                    try:
                        res = requests.post(
                            "http://launcher:5000/trigger/cowrie", timeout=5
                        )
                        if res.status_code == 200:
                            logger.info(
                                "Cowrie started. Transferring session... (mode=%s)",
                                mode,
                            )
                        else:
                            logger.error(
                                "Failed to start Cowrie (HTTP %s)", res.status_code
                            )
                    ...
                    try:
                        output, cwd = cowrie_connector.replay_history(
                            username, password, history
                        )
                    except Exception:
                        logger.exception(
                            "Cowrie connection failed during replay_history"
                        )
                        chan.send(
                            b"Connection to backend lost. Session terminated.\r\n"
                        )
                        break
                    ...
                    logger.warning(
                        "Unexpected state: cowrie not launched in mode %s", mode
                    )
            ...
            try:
                output, cwd = cowrie_connector.execute_command(
                    cmd, username, password, dir_cmd
                )
            except Exception:
