...
class LineReader:
    def __init__(
        ...
        prompt="",
        history=[],
        cowrie_connector: connect_server.SSHConnector = None,
    ):
        self.chan = chan
    ...
    def handle_tab_completion(self):
        ...
        connector = self.cowrie_connector or connect_server.SSHConnector(
            host="cowrie", port=2222
        )
        cwd = connector.replay_cwd_only(
            username=self.username, password=self.password, history=self.history
        )

        command, output_chars = connector.execute_with_tab(
            cwd, command_with_tab, self.username, self.password
        )

        output_chars_clean = ansi_sequences.strip_ansi_sequences(output_chars)
        completed_command = extract_chars.get_completion_diff(
            command.strip(), output_chars_clean.strip()
        )
