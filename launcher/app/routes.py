...
load_dotenv()
allowed_networks = [
    n.strip() for n in os.getenv("ALLOWED_NETWORKS", "").split(",") if n.strip()
]
...
def get_heralding_logs():
    file_path = os.path.join(
        os.path.dirname(__file__), "/data/heralding/log_session.json"
    )
...
def get_snare_logs():
    file_path = os.path.join(
        os.path.dirname(__file__), "/data/tanner/log/tanner_report.json"
    )
