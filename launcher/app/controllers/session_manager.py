...
linked_map = {"snare": ["tanner_redis", "tanner_phpox", "tanner_api", "tanner"]}
...
class ServiceSession:
    def __init__(self, service_name, linked_services=None, persist=False):
        ...
        self._observer_thread = threading.Thread(
            target=self.session_observer, daemon=True
        )
    ...
    def session_observer(self):
        while not self._stop_observer.is_set():
            ...
            logger.info(
                "Waiting for new session to reactive for %s...", self.service_names
            )
